...
write.csv(lots_data, "data/processed_data/all_lots_attributes.csv", row.names=FALSE)

# ------ Part 2 (The official model part) ----------
lots_data = read.csv("data/processed_data/all_lots_attributes.csv")
...
preds <- predict(logit_model, newdata = lots_data[, -15], type = "response")
...
preds <- predict(logit_model_econ, newdata = lots_data[, -15], type = "response")
confusionMatrix(table(as.numeric(preds>class_percent), reference = lots_data$greened))
...
preds <- predict(logit_model_demo, newdata = lots_data[, -15], type = "response")
confusionMatrix(table(as.numeric(preds>class_percent), reference = lots_data$greened))
...
preds <- predict(logit_model_land, newdata = lots_data[, -15], type = "response")
confusionMatrix(table(as.numeric(preds>class_percent), reference = lots_data$greened))
...
propensity_scores = as.data.frame(predict(logit_model, lots_data[, -15], type="response"))
...
write.csv(matches, "./data/processed_data/vacant_lot_matches.csv")
